  ...
  for (byte i = 0; i < 8; i++) { // loops for the number of stages
    ...
    while (TempTimeLoop) { // loops to adjust time setting
      if (i != 0) {
        ...
        LeggiPulsante(Verso, Timer);
        Set(stagetimeSet, 140, 0, 1, Timer, Verso);//Set(stagetimeSet, 140, 1, 1, Timer, Verso);
  ...
  while (setupLoop) {
    switch (setupMenu) {
      ...
      case (3):
        ...
        //if (btn_Press(Button_dn,50))setupMenu = 0;//4
...
void loop() {
  ...
  switch (mainMenu) {
    ...
    default:
      ...
      if (btn_Press(Button_up, 1500))mainMenu = 4;
      break;
  }
}
